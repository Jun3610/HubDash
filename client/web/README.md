...
| 건강: 식단 입력 (#131에서 변경) | 끼니(아침·점심·저녁·간식)마다 제목 + 탄수·지방·단백질(g)만 입력, 칼로리는 탄수×4 + 지방×9 + 단백질×4로 계산해 저장. 서버에는 끼니 1건 + 항목 1건으로 저장 | 사용자 요청: 음식을 하나씩 적는 게 불편. 서버 모델을 바꾸지 않고 흡수. 예전에 음식별로 적은 끼니는 합쳐서 보여 주고, 수정하면 한 줄로 합침 |
| 건강: 첫 화면 | 통계 탭(최근 14일 탄단지 누적 칼로리, 7일 평균, 체중 30일, 수면 14일, 주간 평균, 이번 주 운동) | 사용자 요청 |
| 식단 목표 | 서버 `/api/health/diet-goal`, 처음엔 전부 비어 있음. 목표가 없으면 막대 대신 점선과 "목표 없음" | 예전 하드코딩 기본값(1500kcal, 탄수 140 이하 등)이 설정한 적 없는 값처럼 보였음 |
| 나트륨 | 입력·표시에서 뺌(서버 필드는 null로 저장) | 사용자 요청 |
| 체중·수면 시각 | 날짜 + 시:분, 하루에 여러 번 재면 그래프는 그날 마지막 값 | 서버 recordedAt 날짜+시각(#137) |
| 홈 KPI | "이번 주 과제 완료율" → "이번 주 운동"(권장 150분 대비) | 과제 제거(#135) |
| 학업: 과목 대시보드 | `/pknu/courses/:id` — 학점·학기·성적·교수, 성적 버튼(누르면 바로 저장), 과목 메모(저장 버튼·⌘S). 과목 표의 행이나 사이드바 과목을 누르면 이동, 과목 수정은 대시보드의 "과목 수정" | 사용자 요청 (#134) |
| 평점 | 4.5 만점(A+ 4.5 … D0 1.0, F 0), 성적 나온 과목만 학점 가중 평균, 학기 평점과 전체 평점 | |
| 과목 수정 요청 | `courseBody()`로 기존 값을 모두 담아 보냄 | 서버 PUT이 빠진 필드를 지움 |
| 브레드크럼 | "HubDash"는 홈, 가운데 칸은 그 화면 첫 화면으로 이동 | 사용자 요청 (#132) |
| 메모 · 습관 | 사이드바 '생활 · 습관'을 없애고 메모 화면의 탭(메모 / 습관 / 독서)으로 합침, `/life`는 `/memo?tab=habits`로 | 사용자 요청 (#136) |
| 메모 저장 | 자동 저장(0.8초)에 더해 저장 버튼과 ⌘S, 상태 "저장 안 됨 · 곧 자동 저장 / 저장 중 / 저장됨" | 저장되는지 알기 어렵다는 피드백 (#136) |
